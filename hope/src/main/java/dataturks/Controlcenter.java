...
//                // 如果后端数据库中 result 这个字段为空，那就直接设置，这里也有可能是 null 值
//                String dbResult = result.getResult();
//                // 判断的时候，要先判断 null 值
//                // 空字符串这个 bug 可能复现不了，但是是存在的
//                if( dbResult== null || dbResult.equals("")){
//                    result.setResult(reqObj.getReqMap().get("result"));
//                }
//                // 如果是未完成的状态，那就对标记结果进行追加（因为我们标记的时候只标记了图片的部分区域）
//                // 存在问题：如果是同一个区域，会进行叠加
//                else if(DConstants.HIT_STATUS_NOT_DONE.equalsIgnoreCase(hitStatus)){
//                    String preResult = result.getResult();
//                    String curResult = reqObj.getReqMap().get("result");
//                    // 为了与原来的格式（JSON数组）保持一致，需要对 [{"label": ["Car"],...] ，我们需要对数据进行处理
//                    // 1、获取原来结果，同时去除最后一个中括号，
//                    // 2、取当前标记的结果，同时去掉第一个中括号
//                    // 3、拼接
//                    result.setResult(preResult.substring(0, preResult.length()-1) + "," + curResult.substring(1));
//                }
                result.setResult(reqObj.getReqMap().get("result"));
