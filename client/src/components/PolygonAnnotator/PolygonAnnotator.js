  ...
  getDecPoints(currentRect) {
    ...
    for (let index = 0; index < currentRect.length; index++) {
      points =
        ...
        ',' +
        Math.ceil(currentRect[index][1] * this.canvas.offsetHeight) +
        ' '
    }
    return points.trim()
  }
    ...
    if (currentRect.length === 1) {
      ...
      return (
        <polyline
          ...
          style={{ fill: 'lightblue', opacity: '0.5', stroke: '#1ae04e', strokeWidth: 1 }}
        />
      )
    }
  }
      ...
      >
        <div>
          <Button
            ...
            color="grey"
            icon
            onClick={this.props.getBackTopreviousRow}
        ...
        <div>
          <Button
            ...
            color="blue"
            icon
            onClick={this.props.saveTagAndNextRow}
            disabled={this.props.currentIndex < 0}
          >
            {nextButton}
        ...
        for (let jindex = 0; jindex < rect.length; jindex++) {
          ...
          pointArrs.push(
            <circle
              id={id}
              style={style}
              cx={Math.ceil(rect[jindex][0] * this.canvas.offsetWidth)}
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {this.props.space && this.props.drawHandle && (
            ...
            >
              <label>
                <span>
                  Contrast : <b> {Math.round(this.state.contrast * 100)}%</b>{' '}
                </span>
                <input
                  type="range"
                  step="0.01"
              ...
              <label>
                <span>
                  Saturation : <b> {Math.round(this.state.saturation * 100)}% </b>{' '}
                </span>
                <input
                  type="range"
                  step="0.01"
              ...
              <label>
                <span>
                  Brightness : <b> {Math.round(this.state.brightness * 100)}% </b>
                </span>
                <input
                  type="range"
                  step="0.01"
          ...
          <div>
            <div style={{ lineHeight: 0, position: 'relative', display: 'block' }}>
              <PanZoomElement
                ...
                drawButton={this.state.drawButton}
                width={windowWidth}
                height={windowHeight}
              >
                <img
                  style={{
                    filter: `contrast(${this.state.contrast}) brightness(${this.state.brightness}) saturate(${this.state.saturation})`,
                    height: '100%',
                    width: '100%',
                    display: 'block',
                  }}
                ...
                {(!this.state.imgLoad || this.props.loading) && (
                  <Dimmer active>
                    <Loader />
                  </Dimmer>
                )}
        ...
        {this.props.drawHandle && this.state.imgLoad && (
          <div>
            <Button
              ...
              icon
              onClick={() => {
                if (this.state.toolbarHidden) this.setState({ toolbarHidden: false })
                else this.setState({ toolbarHidden: true })
              }}
            >
            ...
            {!this.state.toolbarHidden && (
              ...
              >
                {toolCombo && (
                  <p>
                    {' '}
                    Alternate Between Tool using <b>{toolCombo}</b> key.{' '}
                  </p>
                )}
                <div>
                  <Button
                    size="mini"
                    icon
                    color={!this.state.drawButton ? 'blue' : 'grey'}
                    onClick={toggleTool.bind(this, false)}
                  >
                    <Icon name="move" />
                  </Button>
                </div>
                <div>
                  <Button
                    ...
                    color={this.state.drawButton ? 'blue' : 'grey'}
                    icon
                    onClick={toggleTool.bind(this, true)}
                  >
                    <Icon name="pencil" />
                  </Button>
                </div>
