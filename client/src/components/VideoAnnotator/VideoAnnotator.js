...
export default class VideoAnnotator extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.rects && nextProps.rectCatMap && this.props.rects !== nextProps.rects) {
      this.setState({
        ...
        endTimeMap: nextProps.endTimeMap,
        rectTimeMap: nextProps.rectTimeMap,
        rectShapeMap: nextProps.rectShapeMap,
  ...
  getDecPoints(currentRect) {
    ...
    if (currentRect) {
      for (let index = 0; index < currentRect.length; index++) {
        points =
          points +
          Math.ceil(currentRect[index][0] * this.state.videoWidth) +
          ',' +
          Math.ceil(currentRect[index][1] * this.state.videoHeight) +
          ' '
      }
    }
    return points.trim()
  ...
  increasePlayerTime(ts) {
    ...
    if (!this.DrawBoard)
      this.DrawBoard = SVG(this.svgId).size(this.state.videoWidth, this.state.videoHeight)
    ...
    this.state.rects.map((rect, index) => {
      ...
      if (timeMap) {
        startTime = timeMap[0]
        endTime = this.props.endTimeMap[index]
          ? this.props.endTimeMap[index]
          : this.state.player.duration
      }
      console.log(
        'increasePlayerTime looping',
        this.state.player.paused,
        this.state.player.currentTime,
        startTime,
        endTime,
      ...
      if (
        !this.state.player.paused &&
        this.state.player.currentTime >= startTime &&
        this.state.player.currentTime <= endTime &&
        (!(entity in this.state.hideLabelsMap) ||
          (entity in this.state.hideLabelsMap && this.state.hideLabelsMap[entity] === false))
      ) {
        ...
        if (shape === 'rectangle' && currentRectPoints !== undefined) {
          ...
          if (element !== null) {
            if (
              this.state.player.currentTime &&
              timeMap[timeIndex] !== this.state.player.currentTime &&
            ...
            for (let jindex = 0; jindex < currentRectPoints.length; jindex++) {
              ...
              if (
                ...
                timeMap[timeIndex] !== this.state.player.currentTime &&
                timeMap[timeIndex + 1] !== undefined &&
                timeMap[timeIndex] !== undefined &&
                rect[timeIndex] !== undefined &&
                rect[timeIndex + 1] !== undefined &&
                rect[timeIndex].length === rect[timeIndex + 1].length
              ) {
              ...
              currentx =
                ...
                (this.state.player.currentTime -
                  timeMap[timeIndex] +
                  timeTraveled * this.state.player.playbackRate) *
                  (diffx / difft)
              currenty =
                currenty +
                (this.state.player.currentTime -
            ...
            width = width * this.canvas.offsetWidth
            height = height * this.canvas.offsetHeight
            let rectResponse = this.DrawBoard.rect(width, height)
              .id(id)
              .attr({
                x: x,
                y: y,
          ...
          flag = true
          let pol = this.DrawBoard.polygon(points)
            .id(id)
            .attr({
              fill: `${lineColor}`,
              cursor: 'pointer',
  ...
  isSameRect(id, rect) {
    ...
    if (ex === coords.x && ey === coords.y && ew === coords.width && eh === coords.height)
      return true
    return false
  }
  ...
  loadImages() {
    ...
    for (
      ...
      index < this.props.currentIndex + 2 && index < this.props.hits.length;
      index++
    ) {
  ...
  resizeWindow() {
    ...
    if (this.state.videoLoad) {
      this.setState({
        canvasSet: false,
        videoLoad: false,
        video: this.state.video + '?ts=' + new Date(),
      })
  ...
  mousedownHandle(event) {
    ...
    if (this.state.defaultClass) {
      ...
      if (this.state.toolName === 'rectangle') {
        ...
        if (this.state.currentRect.length === 0) {
          ...
          currentRectangle.push([
            this.getPoint(currentPoint[0] / this.state.videoWidth),
            this.getPoint(currentPoint[1] / this.state.videoHeight),
          ])
          currentRectangle.push([
            this.getPoint(currentPoint[0] / this.state.videoWidth),
            this.getPoint(newPoint.y / this.state.videoHeight),
          ])
          currentRectangle.push([
            this.getPoint(newPoint.x / this.state.videoWidth),
            this.getPoint(currentPoint[1] / this.state.videoHeight),
          ])
          currentRectangle.push([
            this.getPoint(newPoint.x / this.state.videoWidth),
          ...
          this.setState(
            {
              rects: [...this.state.rects, [currentRectangle]],
              rectCatMap,
              rectTimeMap,
              mouseHoverMap,
              currentRect: [],
      ...
      if (splits.length >= 3) {
        ...
        if (mouseHoverMap[rectIndex] && mouseHoverMap[rectIndex] === true) {
          ...
          if (event.target.id.endsWith('temp')) {
            ...
            newRectangle.push([
              (ex + ew) / this.canvas.offsetWidth,
              (ey + eh) / this.canvas.offsetHeight,
            ])
            ...
            this.setState({
              ...
              dragTimeIndex: timeIndex + 1,
            })
          } else {
            console.log(
              'drag point is',
              this.state.rects[rectIndex][timeIndex][pointIndex][0] * this.state.imgWidth
            )
            console.log(
              'drag point is',
              this.state.rects[rectIndex][timeIndex][pointIndex][1] * this.state.imgHeight
            )
            this.setState({
              pointDrag: true,
              dragRect: rectIndex,
              dragPoint: pointIndex,
              dragTimeIndex: timeIndex,
            })
          }
        }
        // if (splits.length === 3) {
      ...
      (event.target.nodeName === 'polygon' || event.target.nodeName === 'rect')
    ) {
      const splits = event.target.id.split('--')
      if (splits.length >= 2) {
        ...
        if (mouseHoverMap[rectIndex] && mouseHoverMap[rectIndex] === true) {
          ...
          if (event.target.nodeName === 'rect') {
            ...
            if (!sameRect) {
              ...
              newRectangle.push([ex / this.canvas.offsetWidth, ey / this.canvas.offsetHeight])
              newRectangle.push([
                ex / this.canvas.offsetWidth,
                (ey + eh) / this.canvas.offsetHeight,
              ])
              newRectangle.push([
                (ex + ew) / this.canvas.offsetWidth,
                ey / this.canvas.offsetHeight,
              ])
              newRectangle.push([
                (ex + ew) / this.canvas.offsetWidth,
                (ey + eh) / this.canvas.offsetHeight,
    ...
    if (this.state.toolName === 'polygon' && currentRect.length === 1) {
      ...
      return (
        <polyline
          ...
          style={{
            fill: `${this.props.entityColorMap[this.state.defaultClass]}`,
            opacity: '0.5',
            stroke: '#1ae04e',
            strokeWidth: `${1 / this.state.scale}`,
          }}
      ...
      >
        <div title={prevButton}>
          <Button
            ...
            color="grey"
            icon
            onClick={this.nextElement.bind(this, 'previous')}
        ...
        <div title={skipButton}>
          <Button
            ...
            color="grey"
            icon
            onClick={this.nextElement.bind(this, 'skip')}
            disabled={this.props.currentIndex < 0}
          >
        ...
        <div title={nextButton}>
          <Button
            ...
            color="blue"
            icon
            onClick={this.nextElement.bind(this, 'next')}
            disabled={this.props.currentIndex < 0}
          >
  ...
  render() {
    ...
    const renderRects = (data, event) =>
      this.state.rects.map((rect, index) => {
        ...
        if (
          ...
          console.log(
            'renderRects rectangle rendering 2',
            timeMap,
            timeIndex,
            currentRectPoints,
            this.state.player.currentTime,
          ...
          if (currentRectPoints !== undefined && (!shape || shape === 'polygon')) {
            ...
            // let opacity = 0.2;
            if (
              this.state.player.currentTime === timeMap[timeIndex] ||
              this.state.player.currentTime === endTime
            ) {
              color = 'rgb(80, 90, 206)'
              // opacity = 0.5;
            ...
            for (let jindex = 0; jindex < currentRectPoints.length; jindex++) {
              ...
              console.log(
                'renderRects rectangle rendering 6',
                currentx,
                timeIndex,
                timeTraveled,
                diffx,
            ...
            if (
              ...
              this.state.rfaMap[index] = {
                ...
                diffX: diffX / difft,
                diffY: diffY / difft,
                diffWidth: diffWidth / difft,
                diffHeight: diffHeight / difft,
              }
              if (!this.state.rfa) {
                this.state.rfa = true
                // this.DrawBoard.clear();
            ...
            // }
            console.log(
              'renderRects rectangle rendering 3 increasePlayerTime original 11',
              currentCoords,
              element
            )
            if (
            ...
            return (
              <rect
                ...
                x={currentCoordx}
                y={currentCoordy}
                width={currentCoordWidth}
                height={currentCoordHeight}
                style={{
                  fill: `${lineColor}`,
                  cursor: 'pointer',
                  opacity: '0.5',
          ...
          for (let jindex = 0; jindex < currentRectPoints.length; jindex++) {
            ...
            pointArrs.push(
              <circle
                id={id}
                style={style}
                cx={Math.ceil(cx * this.canvas.offsetWidth)}
    ...
    const getMenuItems = () => {
      ...
      for (const [key, value] of entries(this.props.entityColorMap)) {
        ...
        if (
          ...
          key.includes(this.state.searchQuery)
        ) {
          let bgC = 'white'
          ...
          arrs.push(
            <div
              className="disable_text_highlighting text-center"
            ...
            >
              {key in this.state.hideLabelsMap && this.state.hideLabelsMap[key] === true && (
                <div title="Show Labels" style={{ marginTop: '2px' }}>
                  <Button
                    ...
                    onClick={event => toggleEyeStatus(key, false, event)}
                  >
                    <Icon
                      name="low vision"
                      style={{ color: `${this.props.entityColorMap[key]}` }}
                    />
                  </Button>
                </div>
              )}
              {(!(key in this.state.hideLabelsMap) ||
                (key in this.state.hideLabelsMap && this.state.hideLabelsMap[key] === false)) && (
                <div title="Hide Labels" style={{ marginTop: '2px' }}>
                  <Button
                    size="mini"
                    icon
                    style={{ backgroundColor: 'white', cursor: 'pointer' }}
                    onClick={event => toggleEyeStatus(key, true, event)}
                  >
              ...
              <div style={{ cursor: 'pointer' }}>
                <div>
                  <Label
                    ...
                    style={{
                      ...
                      color: 'white',
                      backgroundColor: `${this.props.entityColorMap[key]}`,
                    }}
                  >
                    {' '}
                    {key}
                ...
                <div>{combo && <p style={{ fontSize: '0.6rem' }}> {combo}</p>}</div>
              </div>
            </div>
          )
          // } else {
    ...
    const getTimePoints = (id, timemap) => {
      ...
      for (let index = 0; index < timemap.length; index++) {
        ...
        if (time !== undefined) {
          renderArrs.push(
            <div>
              <Label style={{ cursor: 'pointer' }} onClick={this.seek.bind(this, time)} size="mini">
                {' '}
                {time}{' '}
              </Label>
            </div>
          ...
          <div className="well compact ui text-center" style={{ position: 'relative' }}>
            ...
            <div className="well compact" style={{ display: 'flex', flexDirection: 'row' }}>
              <div>
                ...
                {this.props.drawHandle &&
                  parseFloat(startTime) - 0.001 * this.state.player.playbackRate > 0 && (
                    <Icon
                      style={{ cursor: 'pointer' }}
                      onClick={setTime.bind(
                        this,
                        id,
                        toFixedDec(parseFloat(startTime) - 0.001 * this.state.player.playbackRate),
                        0
                      )}
                      size="mini"
                      name="minus"
                ...
                <Label
                  style={{ cursor: 'pointer' }}
                  onClick={this.seek.bind(this, startTime)}
                  size="mini"
                >
                  {' '}
                  {startTime}{' '}
                ...
                {this.props.drawHandle &&
                  parseFloat(startTime) + 0.001 * this.state.player.playbackRate <=
                    this.state.player.duration &&
                  endTime !== undefined &&
                  parseFloat(startTime) + 0.001 * this.state.player.playbackRate <= endTime && (
                    <Icon
                      style={{ cursor: 'pointer' }}
                      onClick={setTime.bind(
                        this,
              ...
              {endTime === undefined && (
                <Button
                  style={{ cursor: 'pointer' }}
                  disabled={this.state.player.currentTime < startTime}
                  compact
                  onClick={saveEndTime.bind(this, id)}
                  size="mini"
                  color="red"
                >
                  {' '}
              ...
              {endTime !== undefined && (
                <div>
                  ...
                  {this.props.drawHandle &&
                    parseFloat(endTime) - 0.001 * this.state.player.playbackRate >= 0 &&
                    parseFloat(endTime) - 0.001 * this.state.player.playbackRate >= startTime && (
                      <Icon
                        style={{ cursor: 'pointer' }}
                        onClick={setTime.bind(
                          this,
                          id,
                          toFixedDec(parseFloat(endTime) - 0.001 * this.state.player.playbackRate),
                          -1
                        )}
                        size="mini"
                        name="minus"
                  ...
                  <Label
                    style={{ cursor: 'pointer' }}
                    onClick={this.seek.bind(this, endTime)}
                    size="mini"
                  >
                    {' '}
                    {endTime}{' '}
                  ...
                  {this.props.drawHandle &&
                    parseFloat(endTime) + 0.001 * this.state.player.playbackRate <=
                      this.state.player.duration && (
                      <Icon
                        style={{ cursor: 'pointer' }}
                        onClick={setTime.bind(
    ...
    const renderCompleteLabels = () => {
      ...
      for (const key of Object.keys(this.state.rectTimeMap)) {
        ...
        renderarrs.push(
          <Label
            style={{ cursor: 'pointer', color: 'white', backgroundColor: `${color}` }}
            size={size}
            id={key}
          ...
          >
            {cat}{' '}
            {this.props.drawHandle && (
              <Icon name="delete" id={key} onClick={removeRect.bind(this)} />
            )}
          </Label>
        )
      }
        ...
        >
            ...
            <div style={{ lineHeight: 0, display: 'block', position: 'relative' }}>
              ...
              >
                ...
                >
                  ...
                  />
                  {(this.props.loading || !this.state.videoLoad) && (
                    <Dimmer active>
                      <Loader />
                    </Dimmer>
                  )}
                    ...
                    >
                      {this.canvas && this.canvas.offsetWidth && (
                        ...
                        >
                          ...
                          {this.state.player.paused &&
                            this.props.drawHandle &&
                            Object.keys(this.state.rects).length >= 0 &&
                            renderPoints()}
                          {this.state.currentRect &&
                            this.props.drawHandle &&
                            this.state.currentRect.length > 0 &&
                            this.rectToStyles(this.state.currentRect)}
                          {this.state.player.paused &&
                            this.props.drawHandle &&
                            this.state.currentRect &&
                            this.state.currentRect.length > 1 &&
                            renderCurrentPoints()}
                        </svg>
                      )}
                    </div>
          ...
          {this.state.videoLoad && (
            <div>
              ...
              {!this.state.player.paused && (
                <p
                  ref={ptext => (this.ptextPlay = ptext)}
                  id="progressBarTextPlay"
                  className="text-center"
                  style={{ fontSize: 'xx-small' }}
                >
                  {this.state.player.currentTime} / {this.state.player.duration}
              ...
              <input
                ...
                className="ui range"
                onChange={(event, data) => {
                  console.log('onSeekChange', event.target.value, data)
                  this.seek(event.target.value)
                }}
                min={0}
                step={0.0001}
                max={this.state.player.duration}
                type="range"
              ...
              <Button
                ...
                icon
                size="mini"
                onClick={this.changeCurrentTime.bind(this, -10 / 25)}
              >
                {' '}
              ...
              <Button
                ...
                icon
                size="mini"
                onClick={this.changeCurrentTime.bind(this, -1 / 25)}
              >
                {' '}
                <Icon name="backward" />
              </Button>
              {this.state.player.paused && (
                <Button title="Play" icon size="mini" onClick={this.play}>
                  {' '}
                  <Icon name="play" />
                </Button>
              )}
              {!this.state.player.paused && (
                <Button title="Pause" icon size="mini" onClick={this.pause}>
                  {' '}
                  <Icon name="pause" />
                </Button>
              )}
              ...
              <input
                ...
                className="range"
                onChange={(event, data) => {
                  console.log('change volume', event.target.value, data)
                ...
                step={0.1}
                max={1}
                value={this.state.player.volume}
              />

              <p>
                Playback Speed : <b>{this.state.player.playbackRate}x</b>
              </p>
              <input
                title="PlayBack Rate"
                className="ui range"
                onChange={(event, data) => {
                  console.log('changePlaybackRateRate', event.target.value, data)
                  this.changePlaybackRateRate(parseFloat(event.target.value))
                }}
                min={0}
                step={0.01}
                max={2}
                type="range"
                value={this.state.player.playbackRate}
              />
            </div>
          )}
        </div>
              ...
              >
                {
                  ...
                  >
                    <div title="Click to draw Polygon">
                      <Button
                        size="mini"
                        color={this.state.toolName === 'polygon' ? 'blue' : 'grey'}
                        icon
                        onClick={this.toggleTool.bind(this, 'polygon')}
                      >
                    ...
                    <div title="Click to draw Rectangle">
                      <Button
                        size="mini"
                        color={this.state.toolName === 'rectangle' ? 'blue' : 'grey'}
                        icon
                        onClick={this.toggleTool.bind(this, 'rectangle')}
                      >
                ...
                {this.props.drawHandle && (
                  ...
                  >
                    <label>
                      ...
                      <br />
                      <span>
                        Box Opacity : <b> {Math.round(this.state.opacity * 100)}%</b>{' '}
                      </span>
                    </label>
                  </div>
                )}
                {this.state.rects.length > 0 && (
                  <div
                    id="currentLabel"
                    style={{ marginLeft: '15px', height: '300px', overflow: 'auto' }}
                  >
                    {renderCurrentLabel()}
                  </div>
                )}
                {this.state.rects.length > 0 && (
                  <div
                    id="completeLabel"
                    style={{ height: '100px', marginLeft: '15px', overflow: 'auto' }}
                  >
