...
export const createEntities = ruleLine => {
  ...
  return rules.tags.split(',').map(Function.prototype.call, String.prototype.trim)
}
...
export const getDetaultShortcuts = (type, entities) => {
  ...
  } else if (type === IMAGE_POLYGON_BOUNDING_BOX || type === IMAGE_POLYGON_BOUNDING_BOX_V2) {
...
export const createDocEntityColorMap = entities => {
  ...
  if (entities !== undefined) {
    for (let index = 0; index < entities.length; index++) {
      colorMap[entities[index]] =
        DOC_ENTITY_COLORS[
          index > DOC_ENTITY_COLORS.length ? index % DOC_ENTITY_COLORS.length : index
        ]
    }
  }
  return colorMap
...
export const captureException = message => {
  if (window.Raven) {
    alert(message)
    window.Raven.captureException('Exception-Capture' + JSON.stringify(message))
  }
}
...
export const imageBoundingSample = {
  content: 'https://s3.amazonaws.com/com.dataturks.uploads/airplanes__image_0582.jpg',
  annotation: [
    {
      label: 'Airplane',
      points: [
