...
import TaggerInvite from '../../components/TaggerInvite/TaggerInvite'
...
class ProjectOverview extends Component {
  ...
  constructor(props) {
    super(props)
    // console.log('组件初始化的 props 是', props)
  ...
  componentWillMount() {
    // this.setState({ projectDetails: this.props.location.state })
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.currentProject) {
      if (this.props.currentProject !== nextProps.currentProject) {
        ...
        fetchHitsDetails(nextProps.currentProject, 0, 20, this.hitsFetched, 'done')
      }
      if (this.props.currentProject === nextProps.currentProject && !this.state.projectDetails) {
        this.loadProjectDetails(nextProps.currentProject)
      }
      if (this.props.currentProject === nextProps.currentProject && !this.state.hitsDetails) {
        fetchHitsDetails(nextProps.currentProject, 0, 20, this.hitsFetched, 'done')
      }
    }
  }
  ...
  close = () => {
    ...
    if (this.state.redirectToHome) {
      this.props.history.push('/userHome/my-projects')
    }
  }

  openExport = () => {
    // 路由暂时未定义
    this.props.history.push('/userHome/projects/export')
  }
  ...
  openScreen = (screen, type) => {
    ...
    if (screen === 'edit') {
      this.props.history.push({
        pathname: '/userHome/projects/' + orgName + '/' + name + '/' + 'edit' + '/' + type,
        //query: { type },
  ...
  inviteSent(error, response) {
    ...
    if (!error) {
      ...
      this.setState({
        successModal: true,
        successMessage:
          'Successfully added to project, please ask them to sign up to see the project',
      })
    } else {
  ...
  showTags = entiti => {
    ...
    for (let index = 0; index < entiti.length; index++) {
      ...
      if (this.state.label !== entity) {
        onClickFn = () => {
          ...
          fetchHitsDetails(this.props.currentProject, 0, 20, this.hitsFetched, 'done', entity)
        }
      } else {
        onClickFn = () => {
          this.state.start = 0
    ...
    return (
      <div style={{ maxHeight: '10em', display: 'flex', flexWrap: 'wrap', overflow: 'auto' }}>
        {renderArrs}
      </div>
    )
  }
  ...
  nextElement(event) {
    console.log('event', event)
    if (this.state.start < this.state.count && this.state.start < this.state.hitsDetails.length) {
      this.setState({ start: this.state.start + 1 })
    } else {
      const newCount = this.state.currentCount + this.state.count
      this.loadProjectDetails(newCount)
    }
  }
    ...
    try {
      ...
      for (let index = 0; index < words.length; index++) {
        ...
        if (word.length > 0) {
          ...
          renderArrs.push(
            <Label name={index} style={{ color: 'white', backgroundColor: color }} key={index}>
  ...
  showExtra = extra => {
    ...
    return (
      <STable celled color="blue" key="blue" size="small" compact="very" collapsing>
  ...
  render() {
    // console.log('开始渲染，属性@', this.props, '状态@', this.state)
    const { projectDetails, successModal, hitsDetails, entities } = this.state
    // console.log('hitsDetails', hitsDetails)
    // console.log('projectDetails', projectDetails)
    if (!projectDetails) return <div></div>
    let permissions = projectDetails.permissions === null ? {} : projectDetails.permissions
    let createdDate = projectDetails.created_timestamp
      ? timeConverter(projectDetails.created_timestamp / 1000)
      : ''
    const taggingProgress =
      projectDetails.totalHits === 0
        ? 0
        : ((projectDetails.totalHitsDone * 100) / projectDetails.totalHits).toFixed(0)
    ...
    // }
    const path = '/userHome/projects/' + projectDetails.orgName + '/' + projectDetails.name + '/'
    ...
    return (
      <div>
        ...
        {projectDetails && (
          <div>
            <Segment basic vertical size="large" className={styles.title}>
              <div
              ...
              >
                <Breadcrumb size="big">
                  ...
                  <Breadcrumb.Divider />
                  <Breadcrumb.Section active>{projectDetails.name}</Breadcrumb.Section>
                </Breadcrumb>
                <div />
                {/*下拉选择对应功能*/}
                <div style={{ marginRight: '50px' }}>
                  <Dropdown text="Options" icon="options" labeled button className="icon mini teal">
                    <Dropdown.Menu>
                      <Dropdown.Item
                      ...
                      <Dropdown.Item
                        // disabled={!permissions.canSeeCompletedHITs}
                        onClick={this.openScreen.bind(this, 'overview', 'skipped')}
                      >
                      ...
                      <Dropdown.Item
                        onClick={() =>
                          downloadZip(this.state.hitsDetails, this.state.entityColorMap)
                        }
                      >
                        <Icon name="download" color="blue" />
                        Download in Zip
        ...
        {this.state.showDeleteConfirmation && (
          <div>
            <Modal.Dialog>
              ...
              <Modal.Body>
                Once deleted we would not be able to recover the data. Please make sure you have a
                backup at your end for all the data.
              </Modal.Body>
              <Modal.Footer>
                <Button
                  onClick={() => {
            ...
            >
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <Progress
                  ...
                  precision={2}
                  progress="percent"
                  style={{ width: '50%', height: '100%', left: '25%', fontSize: '0.75rem' }}
                />
                <div
                  style={{
                    padding: '0.5rem',
                    borderTop: '1px solid lightsteelbue',
                    borderBottom: '1px solid lightsteelbue',
                    textAlign: 'left',
                    display: 'flex',
                    flexDirection: 'row',
                  ...
                  <div>
                    ...
                    <Statistic.Group size="mini" widths="eight">
                      <Statistic
                        ...
                        size="mini"
                        as="a"
                        href={path + 'space?type=done'}
                        onClick={event => {
                          event.preventDefault()
                          this.openScreen('space', 'done')
                        }}
                      >
                      ...
                      <Statistic
                        ...
                        onClick={event => {
                          this.openScreen('space', 'skipped')
                          event.preventDefault()
                        }}
                      >
                        <Statistic.Value>{projectDetails.totalHitsSkipped}</Statistic.Value>
                        <Statistic.Label style={statsLabel}>HITs Skipped</Statistic.Label>
                      </Statistic>
                      <Statistic
                        color="red"
                        size="mini"
                        ...
                        onClick={event => {
                          this.openScreen('space', 'deleted')
                          event.preventDefault()
                        }}
                      >
                        <Statistic.Value>{projectDetails.totalHitsDeleted}</Statistic.Value>
                        <Statistic.Label style={statsLabel}>HITs Deleted</Statistic.Label>
                      </Statistic>
                      <Statistic size="mini">
                        <Statistic.Value color="blue">{projectDetails.totalHits}</Statistic.Value>
                    ...
                    <div style={{ textAlign: 'left' }}>
                      ...
                      <Statistic.Group size="mini" widths="four">
                        <Statistic color="green" size="mini">
                          <Statistic.Value>{projectDetails.totalEvaluationCorrect}</Statistic.Value>
                          <Statistic.Label style={statsLabel}>Correct</Statistic.Label>
                        </Statistic>
                        <Statistic color="red" size="mini">
                          <Statistic.Value>
                            {projectDetails.totalEvaluationInCorrect}
                          </Statistic.Value>
                          <Statistic.Label style={statsLabel}>Incorrect</Statistic.Label>
                        </Statistic>
                      </Statistic.Group>
                    </div>
                  )}
        ...
        {projectDetails && (
          ...
          >
            ...
            <span data-for="main" data-tip={disabledError}>
              <Button
                as="a"
                href={path + 'space'}
            ...
            <span data-for="main2" data-tip={disabledError}>
              <Button
                // disabled={!permissions.canInviteCollaborators}
                primary
                size="mini"
            ...
            <ReactTooltip id="main2" disable={permissions.canInviteCollaborators} />
          </div>
        )}
        ...
        {/*      <Segment padded>*/}
        {/*        <h3> Entities </h3>*/}
        {/*        {this.showTags(entities)}                        /!* 展示标注图片的标注类别 *!/*/}
        {/*        {extra && <div>{this.showExtra(extra)}</div>}*/}
        ...
        {projectDetails && hitsDetails && hitsDetails.length >= 0 && (
          <div className="text-md-center text-xs-center">
            <span data-for="main7" data-tip={disabledError}>
              <Button
                disabled={!permissions.canCompleteHITs}
                primary
                onClick={event => {
...
export default connect(
  ...
  state => ({
    ...
    params: state.dataturksReducer.projectDetails,
  }),
  dispatch =>
    bindActionCreators(
      {
