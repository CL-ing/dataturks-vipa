...
export default class TaggerOveriew extends Component {
  ...
  componentDidMount() {
    ...
    if (!this.state.hitsDetails && this.props.currentProject) {
      ...
      this.props.setCurrentProject(
        { orgName: this.props.params.orgName, projectName: this.props.params.projectName },
        getUidToken()
      )
    }
    // this.loadData()
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      (!this.props.projectDetails && nextProps.projectDetails) ||
      ...
      if (
        ...
        nextProps.location.query.contributorId
      ) {
  ...
  getUrl(type, contributorId, entity) {
    ...
    let url =
      '/projects/' + this.props.params.orgName + '/' + this.props.params.projectName + '/overview?'
    if (type) {
      url = url + 'type=' + type
    }
    if (contributorId) {
    ...
    for (let index = 0; index < entiti.length; index++) {
      ...
      if (this.state.label === entity) {
        onClickFn = () => {
          ...
          this.props.pushState(this.getUrl(this.state.type, this.state.contributorId, null))
        }
      } else {
        onClickFn = () => {
          this.state.start = 0
          this.props.pushState(this.getUrl(this.state.type, this.state.contributorId, entity))
        }
      }
      renderArrs.push(
        <Label
          ...
          as="a"
          onClick={() => onClickFn()}
          style={{ padding: '5px', color: 'white', backgroundColor: color }}
          name={entity}
          key={index}
        >
          {entity}
    ...
    return (
      ...
      >
        ...
        <Button
          ...
          size="mini"
          primary
          className={buttonC + ' pull-right'}
          onClick={this.editHit.bind(this, hit)}
        >
          <Icon name="edit" />
        </Button>
      </div>
  ...
  openExport = () => {
    this.props.pushState(
      '/projects/' + this.props.params.orgName + '/' + this.props.params.projectName + '/export'
    )
  }
  ...
  showHits = hitsDetails => {
    ...
    for (let index = 0; index < hitsDetails.length; index++) {
      renderArrs.push(this.showTagLine(hitsDetails[index]))
    }
    return renderArrs
  }
  ...
  showCurrentTags(tags) {
    ...
    for (const ent of tags) {
      ...
      renderArrs.push(
        <Label
          key={ent}
          style={{ padding: '5px', color: 'white', backgroundColor: entityColorMap[ent] }}
        >
          {' '}
          {ent}
        </Label>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <div>
          <Button
            ...
            icon
            labelPosition="left"
            onClick={this.editElement.bind(this, 'edit')}
          >
            <Icon name="edit" />
            edit
          </Button>
        ...
        {this.state.type === 'skipped' && (
          <div>
            <Button
              ...
              icon
              labelPosition="left"
              onClick={this.moveToDone.bind(this, 'move')}
            >
              <Icon name="check" />
              Move to Completed
            </Button>
        ...
        <div>
          <Button
            ...
            disabled={
              this.state.start < 0 ||
              (this.state.scrollCompleted && this.state.start === this.state.hitsDetails.length - 1)
            }
          >
            Next
  ...
  showBoundedImages = hitsDetails => {
    ...
    if (
      currentHit.hitResults &&
      currentHit.hitResults.length > 0 &&
      currentHit.hitResults[0] &&
      currentHit.hitResults[0].notes
    ) {
  ...
  render() {
    ...
    if (
      ...
      hitsDetails.length > this.state.start &&
      this.state.start >= 0 &&
      ...
      <div className="taggerPages">
              ...
              <div>
                ...
                <div className="text-center col-md-6">
                  <Breadcrumb size="big">
                    <Breadcrumb.Section
                      link
                      onClick={() => this.props.pushState('/projects/' + this.props.params.orgName)}
                    >
                      {this.props.params.orgName}
                    </Breadcrumb.Section>
                    <Breadcrumb.Divider />
                ...
                <div className="col-md-3">
                  {projectDetails &&
                    ...
                    projectDetails.permissions.canDownloadData && (
        ...
        {projectDetails && hitsDetails && hitsDetails.length >= 0 && (
          <div className="marginTopExtra">
            {projectDetails.task_type === POS_TAGGING && hitsDetails && hitsDetails.length > 0 && (
              <div>
                <div
