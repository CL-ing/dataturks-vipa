...
class TaggerEdit extends Component {
  ...
  constructor(props) {
    ...
    const type = this.props.match.params.type;// 参数放在 query 里面，页面刷新之后，query参数会丢失
    ...
    if (this.state.projectDetails) {
      const { name, taskRules } = this.state.projectDetails
  ...
  componentDidMount() {
    console.log(' TaggerEdit 组件加载', this.props)
  ...
  openScreen = screen => {
    ...
    this.props.history.push('/userHome/projects/' + screen)
  }
  ...
  handleSubmit = (response, type) => {
    console.log('添加数据之后，查看状态', this.state)
    console.log('handleSubmit', response, type)
    const { task_type } = this.props.location.state
    ...
    if (this.props.currentProject && this.state.file) {
      ...
      if (this.state.uploadType === 'Pre-Annotated') {
        ...
        if (task_type === DOCUMENT_ANNOTATION || task_type === POS_TAGGING_GENERIC) {
        ...
        rules.classification = [
          {
            ...
            classes: this.state.fields.classification_classes.split(','),
          },
        ]
      }
      if (this.state.projectDetails && task_type === IMAGE_BOUNDING_BOX) {
  ...
  render() {
    ...
    const projectDetails = this.props.location.state
    let submitDisabled = true
    if (projectDetails) {
      if (
        this.state.fields &&
        (Object.keys(this.state.fields).length >= 2 || this.state.file) &&
        !this.state.loading
      ) {
      ...
      if (
        ...
        this.state.fields &&
        Object.keys(this.state.fields).length >= 2 &&
        this.state.fields.projectName.length === 0
      ) {
        submitDisabled = true
      } else if (
        !submitDisabled &&
    ...
    if (
      (this.state.fields.classification_name.length > 0 &&
        (this.state.fields.classification_classes.length === 0 ||
          this.state.fields.classification_disp_name.length === 0)) ||
      ((this.state.fields.classification_name.length === 0 ||
        this.state.fields.classification_disp_name.length === 0) &&
        this.state.fields.classification_classes.length > 0) ||
      ((this.state.fields.classification_name.length === 0 ||
      ...
      <div id="home-page" className="taggerPages">
        {projectDetails && (
          <div className="text-center marginTop">
            <Segment basic size="large" loading={this.state.loading} style={{ display: 'flex' }}>
              <Button
                className="pull-left"
                onClick={() =>
              ...
              (projectDetails.task_type === POS_TAGGING ||
                ...
                <div>
                  ...
                  {!this.state.projectEdited && projectDetails && (
                    <Form size="small" key="import1" loading={this.state.loading} compact="true">
                      <Form.Input
                        style={inputWidth}
                        id="projectName"
                        size="small"
                      ...
                      <Form.Input
                        ...
                        label="List of tags comma seperated"
                        control="input"
                        type="text"
                        value={tags}
                      />
                            ...
                            <Menu.Item>
                              <Accordion.Title
                                ...
                                onClick={() => {
                                  ...
                                  else this.setState({ activeIndex: 0 })
                                }}
                              />
                              <Accordion.Content
                                active={this.state.activeIndex === 0}
                      ...
                      {this.state.type === 'label' &&
                        ...
                        (projectDetails.task_type === DOCUMENT_ANNOTATION ||
                          ...
                          <div style={{ position: 'relative' }} className="ui well">
                            {this.state.addClassificationGroup &&
                              (!this.state.fields.classification_name ||
                                !this.state.fields.classification_disp_name ||
                                !this.state.fields.classification_classes) && (
                                <Label
                                  as="a"
                                  size="mini"
                                  onClick={() => this.setState({ addClassificationGroup: false })}
                                >
                                  <Icon color="blue" size="small" name="minus circle" /> Hide
                                  Classification
                                </Label>
                              )}
                            ...
                            {this.state.fields.classification_name &&
                              ...
                              this.state.fields.classification_classes.length > 0 && (
                                <Label
                                  ...
                                  size="mini"
                                  onClick={() => {
                                    this.setState({ addClassificationGroup: false })
                                    this.resetClassification()
                                  }}
                                >
                                  <Icon color="blue" size="small" name="minus circle" /> Remove
                                  Classification
                                </Label>
                              )}
                            <br />
                            {!this.state.addClassificationGroup &&
                              this.state.fields.classification_name.length === 0 && (
                                <Label
                                  as="a"
                                  size="mini"
                                  onClick={() => this.setState({ addClassificationGroup: true })}
                                >
                                  <Icon color="blue" size="small" name="plus circle" /> Add
                                  Classification
                                </Label>
                              )}
                            {(this.state.addClassificationGroup ||
                              (this.state.fields.classification_name &&
                                this.state.fields.classification_name.length > 0)) && (
                              <Form compact="true" size="mini">
                                <h5> Classification Options </h5>
                                <Form.Input
                                  style={inputWidth}
                                  onChange={this.handleChange.bind(this, 'classification_name')}
                                  id="classification_name"
                                  compact="true"
                                  label="Name"
                                  value={this.state.fields.classification_name}
                                  placeholder="Theme"
                                />
                                <Form.Input
                                  style={inputWidth}
                                  ...
                                  compact="true"
                                  label="Display Name"
                                  value={this.state.fields.classification_disp_name}
                                  placeholder="Choose the theme for given aritcle."
                                />
                                <Form.Input
                                  style={inputWidth}
                                  onChange={this.handleChange.bind(this, 'classification_classes')}
                                  id="classification_classes"
                                  compact="true"
                                  label="Classes"
                                  value={this.state.fields.classification_classes}
                                  placeholder="News,Sports,Business"
                                />
                              </Form>
                            )}
                          </div>
                      ...
                      <br />
                      <Button type="submit" disabled={submitDisabled} onClick={this.handleSubmit}>
                        Submit
                      </Button>
                  ...
                  <Segment basic>
                    <Card.Group>
                      <Card
                        ...
                        centered
                        raised
                        style={{ cursor: 'pointer', height: '80px', textAlign: 'center' }}
                        color="yellow"
                      ...
                      <Card
                        ...
                        description={[
                          ...
                          ' want to go through annotations and correct them.',
                        ].join('')}
                      />
                    </Card.Group>
                  </Segment>
                ...
                <div>
                  ...
                  {(projectDetails.task_type === POS_TAGGING ||
                    ...
                    <div>
                      <p>
                        ...
                        following json format. Format is similar to the annotated and downloaded
                        json file from dataturks. Max size 10MB
                      </p>
                      <pre>
                        <code>{JSON.stringify(posSample)}</code>
                      </pre>
                      <p>
                        <b>Content</b> contains input text, <b>annotation</b> has the labeled
                        content, <b>extras</b> is for some extra columns that you want to show with
                        each row.
                      </p>
                    </div>
                  )}
                  {(projectDetails.task_type === TEXT_SUMMARIZATION ||
                    ...
                    <p>
                      ...
                      <pre>
                        ...
                        <br />
                        <br />
                        AFP - India's Tata Iron and Steel Company Ltd. Tata Iron and Steel Company.
                        id=1 content=games
                        <br />
                  ...
                  {projectDetails.task_type === TEXT_CLASSIFICATION &&
                    this.state.selectedFormat === 'json' && (
                      <p>
                        Please upload a text file with each line in file having input sentence in
                        json format. This is same as download format from dataturks Max size 10MB
                  ...
                  {projectDetails.task_type === TEXT_CLASSIFICATION &&
                    this.state.selectedFormat === 'tsv' && (
                      <p>
                        ...
                        following tab seperated format. Max size 10MB
                        <pre>
                          Text Line Comma Separated Labels Extra KeyValue Pair1(optional) Extra
                          KeyValue Pair1(optional)
                          <br />
                          <br />
                          <br />
                          <br />
                          AFP - India's Tata Iron and Steel Company Ltd. Class1, Class2 id=1
                          content=games
                          <br />
                  ...
                  {projectDetails.task_type === IMAGE_CLASSIFICATION && (
                    <p>
                      ...
                      following tab seperated format. Max size 10MB
                      <pre>
                        Image_URL Comma Separated Labels Extra KeyValue Pair1(optional) Extra
                        KeyValue Pair1(optional)
                        <br />
                        <br />
                        <br />
                  ...
                  {projectDetails.task_type === VIDEO_CLASSIFICATION && (
                    <p>
                      ...
                      following tab seperated format. Max size 10MB
                      <pre>
                        Video_URL Comma Separated Labels Extra KeyValue Pair1(optional) Extra
                        KeyValue Pair1(optional)
                        <br />
                        <br />
                        <br />
                  ...
                  {projectDetails.task_type === IMAGE_BOUNDING_BOX && (
                    <div>
                      ...
                      <pre>
                        <code>{JSON.stringify(imageBoundingSample)}</code>
                      </pre>
                      <p>
                        <b>Content</b> contains input image URL, <b>annotation</b> has the left-top
                        and right-bottom co-ordinates, <b>extras</b> is for some extra columns that
                        you want to show with each row.
                      </p>
                    </div>
                  )}
                  {(projectDetails.task_type === IMAGE_POLYGON_BOUNDING_BOX ||
                    ...
                    <div>
                      ...
                      <p>
                        ...
                        polygon [first and last value of array is of same co-ordinate],{' '}
                  ...
                  <div className="col-md-3 text-center">
                    ...
                    >
                      ...
                      ></input>
                      {this.state.file && this.state.file.size > 10000000 && (
                        <p style={{ color: 'red', fontWeight: 'bold' }}>
                          {' '}
                          File size is big: {bytes(this.state.file.size)}{' '}
                        </p>
                      )}
                      {this.state.file && this.state.file.size < 10000000 && (
                  ...
                  <Button
                    ...
                    onClick={this.handleSubmit.bind(this, 'PRE_TAGGED_JSON')}
                  >
                    Submit
                  </Button>
                  <br />
                  {this.state.fileUploadProgress > 0 && (
                    <Progress
                      percent={Math.floor(this.state.fileUploadProgress)}
                      progress
                      autoSuccess
                    />
                  )}
            ...
            {!this.state.fileUploaded &&
              ...
              projectDetails && (
                <div>
                  ...
                  {projectDetails.task_type === TEXT_SUMMARIZATION && (
                    <p>
                      Please upload a text file with each line in file having sentence to be
                      summarized.
                      <br />
                      <strong> OR </strong> <br />A zip file of all the text documents to be
                  ...
                  {projectDetails.task_type === DOCUMENT_ANNOTATION && (
                    <div>
                      <p>
                        ...
                        <br />
                        <strong> OR </strong> <br />A zip file of all the text/pdf/doc documents.
                  ...
                  {projectDetails.task_type === IMAGE_CLASSIFICATION && (
                    <p>
                      ...
                      <strong> OR </strong> <br />A zip file of all the images. Max file size is 100
                      MB for free plans
                    </p>
                  )}
                  {projectDetails.task_type === VIDEO_CLASSIFICATION && (
