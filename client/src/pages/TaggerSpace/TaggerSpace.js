...
export default class TaggerSpace extends Component {
  ...
  constructor(props) {
    ...
    if (
      ...
      let shortcuts = getDetaultShortcuts(this.props.projectDetails.task_type, entities)
      ...
      if (this.props.currentHit.hitResults && this.props.currentHit.hitResults.length > 0) {
        ...
        if ('classification' in rules) {
          ...
          if (ch.hitResults) {
            ...
            if ('annotationResult' in resultObject) {
              ...
              classificationResponse = getClassificationResponse(resultObject.classificationResult)
            } else {
  ...
  componentDidMount() {
    ...
    if (
      (orgName &&
        projectName &&
        (!this.props.projectDetails ||
          this.props.projectDetails.name !== projectName ||
          this.props.projectDetails.orgName !== orgName)) ||
  ...
  setTagClick(event, data) {
    ...
    if (this.state.tagSelected && this.state.selectIds.length > 0) {
      ...
      for (let index = 0; index < selectIds.length; index++) {
        currentClickState[selectIds[index]] = this.state.entityColorMap[data.name]
    ...
    for (let index = 0; index < classification.length; index++) {
      ...
      for (let jindex = 0; jindex < classes.length; jindex++) {
        ...
        renderArrs.push(
          <Form.Field
            checked={checked}
            onChange={this.handleClassificationCheckBoxChange.bind(this, name, classes[jindex])}
  ...
  getUrl(type, contributorId, entity, evaluationType) {
    ...
    const orgName = this.props.match.params.orgName
    const projectName = this.props.match.params.projectName
    let url = '/projects/' + orgName + '/' + projectName + '/space?'
    if (type) {
      url = url + 'type=' + type
    }
    if (contributorId) {
  ...
  getHitStates() {
    ...
    options.push({
      ...
      onClick: () => {
        ...
        this.resetFilterState()
        this.props.pushState(this.getUrl(HIT_STATE_SKIPPED))
      },
    })
    options.push({
      text: 'Deleted HITs',
      ...
      onClick: () => {
        ...
        this.resetFilterState()
        this.props.pushState(this.getUrl(HIT_STATE_DELETED))
      },
    })
    options.push({
      ...
      onClick: () => {
        ...
        this.resetFilterState()
        this.props.pushState(this.getUrl(HIT_STATE_PRE_TAGGED))
      },
    })
    options.push({
      ...
      onClick: () => {
        ...
        this.resetFilterState()
        this.props.pushState(this.getUrl(HIT_STATE_REQUEUED))
      },
    })
  ...
  getEvaluations() {
    ...
    options.push({
      ...
      onClick: () => {
        ...
        this.state.label = undefined
        this.state.contributorId = undefined
        this.props.pushState(this.getUrl(this.state.type, undefined, undefined, 'NONE'))
      },
    })
    options.push({
      text: 'All',
      ...
      onClick: () => {
        ...
        this.props.pushState(this.getUrl(this.state.type, undefined, undefined, undefined))
      },
    })
    return (
      <Dropdown
    ...
    if (contributorDetails) {
      ...
      options.push({
        ...
        onClick: () => {
          ...
          this.state.contributorId = undefined
          this.props.pushState(
            this.getUrl(this.state.type, undefined, this.state.label, this.state.evaluationType)
          )
        },
      })
    ...
    if (hit && hit.hitResults && this.state.contributorDetailsMap) {
      ...
      return (
        <div>
          <Card
            color="teal"
            style={{ minHeight: '15em', maxHeight: '15em', overflowY: 'auto', overflowX: 'hidden' }}
          >
            <Card.Content>
  ...
  getCurrentResult() {
    ...
    if (this.state.projectDetails.task_type === POS_TAGGING) {
      ...
      for (let index = 0; index < this.state.words.length; index++) {
        if (this.state.taggedEntity[index] && this.state.taggedEntity[index] !== '__') {
          tagString =
            tagString + ' ' + this.state.words[index] + '____' + this.state.taggedEntity[index]
        } else {
        ...
        if (
          ...
          if (window.Raven) {
            ...
            window.Raven.captureException(
              'Error while annotating image, hit is :  ' + JSON.stringify(this.state.currentHit)
            )
          }
        }
      }
      let resultString = ''
      if (this.state.classification && this.state.classification.length > 0) {
        const classificationResult = getClassificationResult(this.state.classificationResponse)
        const resultObject = {
  ...
  moveToDoneCallback(err, response) {
    ...
    if (!err) {
      ...
      this.hitAddCallback(
        undefined,
        'Hit moved to done',
        this.state.action ? this.state.action : 'moveToDone'
      )
    } else {
      this.setState({ loading: false })
      ...
      if (response.body.hits.length === 0 || !currentHit) {
        ...
        if (projectDetails.task_type === POS_TAGGING) {
          if (response.body.hits.length > 0) {
            this.addHitinState(
              currentHit.hitResults ? currentHit.hitResults[0].result : currentHit.data,
              currentHit.data
            )
            this.setState({
              startTime: new Date().getTime(),
          ...
          if (response.body.hits.length > 0) {
            let currentTags = new Set()
            let currentNote = ''
            if (currentHit.hitResults !== null && currentHit.hitResults[0].result.length > 0) {
              const result = currentHit.hitResults[0].result
              try {
                const resultJson = JSON.parse(result)
                currentTags = new Set(resultJson.labels)
  ...
  tagAreaClick(index, keyFlag) {
    ...
    if (keyFlag) {
      for (let jindex = 0; jindex < Object.keys(currentClickState).length; jindex++) {
  ...
  skipRow(event, data) {
    ...
    skipHits(this.state.currentHit.id, this.props.currentProject, this.hitAddCallback)
    return false
  }
  ...
  showTagLine() {
    ...
    for (let index = 0; index < splits.length; index++) {
      ...
      let iconC = 'hidden'
      if (this.state.clickedColor[index] && this.state.clickedColor[index] !== '') {
        iconC = ''
      }
      renderArrs.push(
        <Label
          ...
          key={index}
        >
          <span onClick={this.tagAreaClick.bind(this, index, false)} name={index} key={index}>
            {splits[index]}
          </span>
          {this.state.taggedEntity[index] !== '__' && (
            <p style={{ display: 'inline', fontSize: '1rem' }}>
              &nbsp; &nbsp; &nbsp; {this.state.taggedEntity[index]}
              <Icon name="delete" onClick={this.removeTag.bind(this, index)} className={iconC} />
            </p>
          )}
        </Label>
      )
    ...
    return (
      <div className={styles.tagArea}>
        {renderArrs}
        <Button className={styles.copyButton} size="small" onClick={this.copyToClipboard}>
  ...
  showTags(entiti) {
    ...
    for (let index = 0; index < entiti.length; index++) {
      ...
      const color =
        ENTITY_COLORS[index > ENTITY_COLORS.length ? index - ENTITY_COLORS.length : index]
      let combo = ''
  ...
  annotateCallback = obj => {
    ...
    for (let index = 0; index < obj.annotations.length; index++) {
      ...
      if (!this.state.currentHit.isURL) {
        const selectedString = data.substring(annotation.start, annotation.end + 1)
  ...
  showDocumentAnnotation() {
    ...
    if (this.state.boundingBoxMap) {
      for (let index = 0; index < boundingBoxMap.length; index++) {
        ...
        console.log('doc annotate callback', bb.points[0].start + '-' + bb.points[0].end)
        annotations.push({
          category: bb.label,
    ...
    return (
      <div>
        <PolygonAnnotatorV2
          ...
          image={data}
          drawHandle={
            this.isHitted() || this.state.type === HIT_STATE_NOT_DONE ? this.drawPolygon : undefined
          }
          entityColorMap={this.state.entityColorMap}
    ...
    return (
      <div>
        <VideoAnnotator
          ...
          video={data}
          drawHandle={
            this.isHitted() || this.state.type === HIT_STATE_NOT_DONE ? this.drawVideo : undefined
          }
          entityColorMap={this.state.entityColorMap}
        />
      </div>
    ...
    return (
      <div className={styles.tagArea} style={{ lineHeight: '1.0rem' }}>
        <BoxAnnotator
          ...
          image={data}
          drawHandle={
            this.isHitted() || this.state.type === HIT_STATE_NOT_DONE ? this.drawHandle : undefined
          }
          entityColorMap={this.state.entityColorMap}
        />
      </div>
  ...
  showClassifyTags(flag, direct) {      // 图像分类任务右侧的select label 工具栏
    ...
    for (let index = 0; index < entiti.length; index++) {
      if (
        ...
        entiti[index].toUpperCase().includes(this.state.searchQuery.toUpperCase())
      ) {
        const entity = entiti[index]
        let combo = ''
      ...
      <div>
        ...
        >
          ...
          {(this.state.type === HIT_STATE_SKIPPED ||
            ...
            this.state.type === HIT_STATE_PRE_TAGGED ||
            this.state.type === HIT_STATE_REQUEUED) && (
            <Button compact size="mini" color="red" icon onClick={this.deleteItem.bind(this)}>
              <Icon name="delete" />
              Delete
            </Button>
          )}
    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <Button.Group size="mini">
          <Button onClick={this.evaluateHit.bind(this, 'incorrect')} color="blue">
            Incorrect
          </Button>
          <Button.Or />
          <Button onClick={this.evaluateHit.bind(this, 'correct')} color="blue">
            Correct
          </Button>
        </Button.Group>
      </div>
  ...
  isHitted() {
    return (
      ...
      this.state.type === HIT_STATE_SL ||
      this.state.type === HIT_STATE_AL
    )
  }
        ...
        <div>
          {this.state.type === 'notDone' && (
            <Segment color="green" className="marginTopExtra">
              {this.state.type && this.state.type === 'skipped' && (
                <h4> All skipped rows are tagged, You can check the project stats. </h4>
              )}
              {this.state.type && this.state.type !== 'skipped' && (
                <h4> All rows are tagged, You can check the project stats. </h4>
              )}
              {!this.state.type && (
          ...
          <div style={{ display: 'flex', flexDirection: 'row' }}>
            ...
            >
                  ...
                  {this.state.type !== HIT_STATE_NOT_DONE &&
                    this.props.projectDetails.task_type !== POS_TAGGING && (
                      ...
                      >
                        {this.getHitStates()}
                        <br />
                        {this.isHitted() && this.getEvaluations()}
                        <br />
                        {this.isHitted() &&
                          !this.state.evaluationType &&
                ...
                {this.state.type === HIT_STATE_NOT_DONE &&
                  ...
                  !this.state.hitsCompleted && (
                    <div>
                      ...
                      &nbsp; &nbsp;
                      <Button size="mini" icon onClick={() => this.docAnnotator.clearAll()}>
                        Clear All
                      </Button>
                    </div>
                  )}
                    ...
                    {popoverTop && (
                      <div>
                        <OverlayTrigger trigger={['hover']} placement="bottom" overlay={popoverTop}>
                          <Icon size="mini" name="help circle" color="teal" size="large" />
                        </OverlayTrigger>
                      </div>
                    )}
                  </div>
                      ...
                      this.props.projectDetails.task_type !== VIDEO_CLASSIFICATION && (
                        <div>
                          ...
                          >
                            ...
                            >
                              {!this.state.fullScreen && <Icon color="blue" name="expand" />}
                              {this.state.fullScreen && <Icon color="blue" name="compress" />}
                            </Button>
                          </OverlayTrigger>
                        </div>
                      )}
              ...
              {this.state.type === 'notDone' &&
                (this.props.projectDetails.task_type === DOCUMENT_ANNOTATION ||
                  ...
                  <div>
                    ...
                    >
                      <li>
                        Click on the document and then drag to select text and select a label.
                      </li>
                      <li>
                        More queries?{' '}
                        <a href="https://dataturks.com/help/help.php" target="_blank">
              ...
              {this.state.type !== HIT_STATE_NOT_DONE &&
                ...
                this.props.projectDetails && (
                  <div className="text-center">
                    {this.state.hits.length === 0 && <h2> No items to display here </h2>}
                  </div>
                )}

              {this.state.hits && this.state.currentHit && (
                <div
                  style={{
                ...
                >
                  {this.props.projectDetails.task_type === POS_TAGGING && (
                    ...
                    >
                      <div className="marginTopExtra">
                        ...
                        {this.state.loading && this.state.isFullscreenEnabled && (
                          <Segment basic vertical loading />
                        )}
                        <div style={{ height: '20px' }} disabled={this.state.loading} />
                        {this.showButtons()}
                      </div>
                    </Fullscreen>
                  )}
                  {this.state.projectDetails.task_type === TEXT_SUMMARIZATION && (
                    ...
                    >
                      <div className="marginTopExtra">
                        ...
                        <div style={{ height: '20px' }} disabled={this.state.loading} />
                        {this.state.type === 'notDone' && this.showButtons()}
                      </div>
                    </Fullscreen>
                  )}
                  {this.state.projectDetails.task_type === TEXT_CLASSIFICATION && (
                  ...
                  {this.props.projectDetails.task_type === IMAGE_BOUNDING_BOX && (
                    <div>
                      ...
                      >
                        {<div className="col-md-12">{this.showBoundingImages()}</div>}
                      </Fullscreen>
                    </div>
                  )}
