...
import {
  ploadDataForm, setCurrentHit,
  updateProjectDetails, setCurrentProject, getProjectDetails
...
import {
  getDetaultShortcuts, checkVideoURL,
  convertKeyToString, VIDEO_CLASSIFICATION,
  VIDEO_BOUNDING_BOX, IMAGE_CLASSIFICATION,
  POS_TAGGING_GENERIC, DOCUMENT_ANNOTATION,
  IMAGE_POLYGON_BOUNDING_BOX, IMAGE_POLYGON_BOUNDING_BOX_V2,
  IMAGE_BOUNDING_BOX, createEntitiesJson,
  createDocEntityColorMap, TEXT_MODERATION,
  POS_TAGGING, TEXT_SUMMARIZATION, TEXT_CLASSIFICATION
} from '../../helpers/Utils';
    ...
    if (result.length > 0) {
      ...
      if (bbmap) {
        for (let index = 0; index < bbmap.length; index++) {
          ...
          annotations.push({
            category: bb.label, start: bb.points[0].start, end: bb.points[0].end, text: bb.points[0].text, id: bb.points[0].start + '-' + bb.points[0].end,
            color: colors
          });
        }
      }
